...
def main():
    ...
    pygame.quit()
def block():
    global rect, collect, collected
    for x in range(collected):
        if move=='left':
            pygame.draw.rect(screen,WHITE,(currX+((x-1)*HEIGHT),currY,HEIGHT,HEIGHT),0)
        elif move=='right':
            pygame.draw.rect(screen,WHITE,(currX-((x-1)*HEIGHT),currY,HEIGHT,HEIGHT),0)
        elif move=='up':
            pygame.draw.rect(screen,WHITE,(currX,currY+((x-1)*HEIGHT),HEIGHT,HEIGHT),0)
        elif move=='down':
            pygame.draw.rect(screen,WHITE,(currX,currY-((x-1)*HEIGHT),HEIGHT,HEIGHT),0)
